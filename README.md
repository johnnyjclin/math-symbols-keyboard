...
- 分類：常用符號、比較與邏輯、集合、微積分、希臘字母（內容對照 `symbols.docx`，每個符號附中文說明，滑鼠移上去顯示）。
- **保留格式**：貼上時會帶上該符號在 `symbols.docx` 裡的**字型與大小**（例如 `∫` = Cambria Math 16pt、`×` = Times New Roman 12pt）。原理是把帶 CSS 樣式的 HTML 放進剪貼簿，Word / Google Docs 貼上時即套用該字型與大小；貼到純文字欄位時則自動退回純符號。
